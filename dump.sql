...
--
-- Name: minerva_entity_set_run_length; Type: VIEW; Schema: attribute_history; Owner: minerva_writer
--

CREATE VIEW attribute_history.minerva_entity_set_run_length AS
 SELECT min(runs.id) AS id,
    public.first(runs.entity_id) AS entity_id,
    min(runs."timestamp") AS start,
    max(runs."timestamp") AS "end",
    min(runs.first_appearance) AS first_appearance,
    max(runs.modified) AS modified,
    count(*) AS run_length
   FROM ( SELECT t.id,
            t.entity_id,
            t."timestamp",
            t.first_appearance,
            t.modified,
            sum(t.change) OVER w2 AS run
           FROM ( SELECT minerva_entity_set.id,
                    minerva_entity_set.entity_id,
                    minerva_entity_set."timestamp",
                    minerva_entity_set.first_appearance,
                    minerva_entity_set.modified,
                        CASE
                            WHEN ((minerva_entity_set.hash)::text <> (lag(minerva_entity_set.hash) OVER w)::text) THEN 1
                            ELSE 0
                        END AS change
                   FROM attribute_history.minerva_entity_set
                  WINDOW w AS (PARTITION BY minerva_entity_set.entity_id ORDER BY minerva_entity_set."timestamp")) t
          WINDOW w2 AS (PARTITION BY t.entity_id ORDER BY t."timestamp")) runs
  GROUP BY runs.entity_id, runs.run;


ALTER VIEW attribute_history.minerva_entity_set_run_length OWNER TO minerva_writer;

--
-- Name: minerva_entity_set_compacted; Type: VIEW; Schema: attribute_history; Owner: minerva_writer
--

CREATE VIEW attribute_history.minerva_entity_set_compacted AS
 SELECT rl.id,
    rl.entity_id,
    rl.start AS "timestamp",
    rl."end",
    rl.first_appearance,
    rl.modified,
    history.hash,
    ...
    history.fullname,
    history."group",
    history.source_entity_type,
    history.owner,
    history.description,
    history.last_update
   FROM (attribute_history.minerva_entity_set_run_length rl
     JOIN attribute_history.minerva_entity_set history ON (((history.entity_id = rl.entity_id) AND (history."timestamp" = rl.start))))
  WHERE (rl.run_length > 1);


ALTER VIEW attribute_history.minerva_entity_set_compacted OWNER TO minerva_writer;

--
-- Name: minerva_entity_set_compacted_tmp; Type: TABLE; Schema: attribute_history; Owner: minerva_writer
--

CREATE UNLOGGED TABLE attribute_history.minerva_entity_set_compacted_tmp (
    id integer,
...
ALTER TABLE attribute_history.minerva_entity_set_compacted_tmp OWNER TO minerva_writer;
...
--
-- Name: minerva_entity_set_compacted_tmp_entity_id_timestamp_idx; Type: INDEX; Schema: attribute_history; Owner: minerva_writer
--

CREATE INDEX minerva_entity_set_compacted_tmp_entity_id_timestamp_idx ON attribute_history.minerva_entity_set_compacted_tmp USING btree (entity_id, "timestamp");
...
--
-- Name: TABLE minerva_entity_set_run_length; Type: ACL; Schema: attribute_history; Owner: minerva_writer
--

GRANT SELECT ON TABLE attribute_history.minerva_entity_set_run_length TO minerva;


--
-- Name: TABLE minerva_entity_set_compacted; Type: ACL; Schema: attribute_history; Owner: minerva_writer
--

GRANT SELECT ON TABLE attribute_history.minerva_entity_set_compacted TO minerva;


--
-- Name: TABLE minerva_entity_set_compacted_tmp; Type: ACL; Schema: attribute_history; Owner: minerva_writer
--

GRANT SELECT ON TABLE attribute_history.minerva_entity_set_compacted_tmp TO minerva;
