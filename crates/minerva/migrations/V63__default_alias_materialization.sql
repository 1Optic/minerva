-- 2,+26 replaces V46__make_trend_materialization_fault_tolerant.sql 1,+20
CREATE OR REPLACE FUNCTION trend_directory.function_materialization_columns(materialization_id integer)
...
trend_store_part_columns AS (
  ...
  WHERE m.id = $1
  UNION
  -- Add the 'name' column if primary_alias is enabled for the target trend store part
  SELECT 'name' AS name
  FROM trend_directory.materialization m
  JOIN trend_directory.trend_store_part tsp ON tsp.id = m.dst_trend_store_part_id
  WHERE m.id = $1 AND tsp.primary_alias
)
SELECT f.name
