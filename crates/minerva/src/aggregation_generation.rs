...
fn write_time_aggregations(
    ...
    Ok(TrendStore {
        ...
        partition_size: granularity_to_partition_size(aggregation.granularity).unwrap(),
        retention_period: humantime::parse_duration("1y").unwrap(),
        parts: target_trend_store_parts,
    })
}

pub fn granularity_to_partition_size(granularity: Duration) -> Option<Duration> {
        ...
        PARTITION_SIZE_MAPPING.get_or_init(|| {
            vec![
                ...
                (
                    ...
                    humantime::parse_duration("2d").unwrap(),
                ),
                (
                    Duration::from_secs(3600),
                    humantime::parse_duration("4d").unwrap(),
                ),
                (
                    Duration::from_secs(86400),
    ...
    partition_size_mapping.get(&granularity).cloned()
}
...
fn define_function_part_entity_aggregation(
    ...
    let materialization_attributes = TrendMaterializationAttributes {
        ...
        processing_delay: Duration::from_secs(1800),
        stability_delay: Duration::from_secs(300),
        reprocessing_period: humantime::parse_duration("3d").unwrap(),
        description: None,
        old_data_threshold: config.old_data_threshold,
        old_data_stability_delay: config.old_data_stability_delay,
    };
    ...
    TrendFunctionMaterialization {
        ...
        sources: vec![
            ...
            TrendMaterializationSource::Relation(TrendMaterializationRelationSource {
